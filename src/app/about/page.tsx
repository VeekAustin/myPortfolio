...
export default function About() {
  return (
    <div className="min-h-screen bg-[#0d1117] px-6 py-24">
      <div className="max-w-4xl mx-auto">
        <p className="text-xs tracking-[0.2em] uppercase text-[#238636] font-medium mb-3">
          The person behind the code
        </p>
        <h1 className="text-[clamp(2rem,5vw,3.5rem)] font-extrabold tracking-tight text-[#f0f6fc] mb-12">
          About Me
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
          <div>
            <p className="text-base text-[#8b949e] leading-[1.85] mb-5">
          ...
          <div className="bg-[#0f1a11] border border-[#1a2e1e] rounded-xl p-6">
            <p className="text-xs tracking-[0.15em] uppercase text-[#484f58] mb-4">Quick facts</p>
            {facts.map(([label, value]) => (
              <div
                key={label}
                className="flex justify-between py-2.5 border-b border-[#1a2e1e] last:border-0"
              >
                <span className="text-sm text-[#484f58]">{label}</span>
                <span className="text-sm text-[#238636] font-medium">{value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Timeline */}
        <div>
          <p className="text-xs tracking-[0.15em] uppercase text-[#484f58] mb-6">Journey</p>
          <div className="flex flex-col">
            {timeline.map((item) => (
              <div key={item.year} className="grid grid-cols-[72px_1fr] gap-6 pb-7">
                <span className="text-sm font-bold text-[#238636] pt-0.5">
                  {item.year}
                </span>
                <p className="text-sm text-[#8b949e] leading-relaxed border-l border-[#1a2e1e] pl-6">
                  {item.event}
                </p>
              </div>
            ))}
